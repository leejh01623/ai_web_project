...
window.addEventListener('touchend', (e) => {
	if(isDrawing === true){
		//drawLine(ctx, x, y, offsetX, e.offsetY);
		x = 0;
		y = 0;
		isDrawing = false;
	}
...
function setPcOrMobileFontSize(div, num1, num2){
	if(matchMedia("screen and (max-width: 700px)").matches){
		// 700px 이하 일 때
		div.style.fontsize = num1 + "px";
	} else {
		// 700px 이상 일 때	
		div.style.fontsize = num2 + "px";
	}
}
...
function setDayFunc(){
	...
	if(weatherVal == ""){
		dataStr += "<span class='main-top-date-text'>모름</span>";
	} else {
		dataStr += "<div class='main-top-date-weather'>";
		dataStr += "<img src='./img/" + weatherVal +".png' style='width: 100%; height: 100%;'>";
		dataStr += "</div>";
	}
	dataStr += "</div>";
